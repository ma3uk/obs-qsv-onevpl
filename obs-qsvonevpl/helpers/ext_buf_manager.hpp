...
public:
  ...
  ExtBufManager &operator=(const T &ref) {
    ...
    *dst_base = *src_base;
    ...
    for (size_t i = 0; i < ref_.NumExtParam; ++i) {
      ...
      memcpy(reinterpret_cast<void *>(dst_buf),
             reinterpret_cast<void *>(src_buf), src_buf->BufferSz);
    }
  ...
  template <typename TB> TB *GetExtBuffer(uint32_t fieldId = 0) const {
    return reinterpret_cast<TB *>(FindExtBuffer(mfx_ext_buffer_id<TB>::id, fieldId));
  }

  template <typename TB> operator TB *() {
    return reinterpret_cast<TB *>(FindExtBuffer(mfx_ext_buffer_id<TB>::id, 0));
  }
...
private:
  ...
  bool IsCopyAllowed(mfxU32 id) {
    static const mfxU32 allowed[] = {MFX_EXTBUFF_CODING_OPTION,
  ...
  struct CmpExtBufById {
    ...
    bool operator()(mfxExtBuffer *b) const { return (b && b->BufferId == id); };
  };
