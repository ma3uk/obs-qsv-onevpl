...
public:
	...
	virtual mfxStatus
	GetSurfaceForEncode(mfxFrameSurface1 **output_surf) = 0;
	virtual mfxStatus
	GetSurfaceForDecode(mfxFrameSurface1 **output_surf) = 0;
	virtual mfxStatus GetSurfaceForVPP(mfxFrameSurface1 **output_surf) = 0;
	virtual mfxStatus
	GetSurfaceForVPPOut(mfxFrameSurface1 **output_surf) = 0;
...
public:
	...
	virtual mfxStatus Query(mfxVideoParam *in, mfxVideoParam *out) = 0;
	virtual mfxStatus DecodeHeader(mfxBitstream *bs,
				       mfxVideoParam *par) = 0;
	virtual mfxStatus QueryIOSurf(mfxVideoParam *par,
				      mfxFrameAllocRequest *request) = 0;
	virtual mfxStatus Init(mfxVideoParam *par) = 0;
...
public:
	...
	virtual mfxStatus ProcessFrameAsync(mfxFrameSurface1 *in,
...
public:
	...
	virtual mfxStatus
	SetFrameAllocator(mfxFrameAllocator *allocator) override
	{
	...
	virtual mfxStatus
	GetSurfaceForEncode(mfxFrameSurface1 **output_surf) override
	{
		return MFXMemory_GetSurfaceForEncode(m_session, output_surf);
	}
	virtual mfxStatus
	GetSurfaceForDecode(mfxFrameSurface1 **output_surf) override
	{
		return MFXMemory_GetSurfaceForDecode(m_session, output_surf);
	}
	virtual mfxStatus
	GetSurfaceForVPP(mfxFrameSurface1 **output_surf) override
	{
		return MFXMemory_GetSurfaceForVPP(m_session, output_surf);
	}
	virtual mfxStatus
	GetSurfaceForVPPOut(mfxFrameSurface1 **output_surf) override
	{
		return MFXMemory_GetSurfaceForVPPOut(m_session, output_surf);
	}

	virtual operator mfxSession(void) override { return m_session; }
...
protected:
	...
	mfxStatus CreateConfig(varDataType data, const char *propertyName)
	{
		...
		return MFXSetConfigFilterProperty(cfg, (mfxU8 *)propertyName,
						  variant);
	}
	...
	mfxStatus InitSession(mfxInitParam par)
	{
		...
		case MFX_IMPL_SOFTWARE:
			mfxRes =
				CreateConfig<mfxU32>(MFX_IMPL_TYPE_SOFTWARE,
		...
		case MFX_IMPL_HARDWARE4:
			mfxRes =
				CreateConfig<mfxU32>(MFX_IMPL_TYPE_HARDWARE,
						     "mfxImplDescription.Impl");
			break;
		...
		switch (implBaseType) {
		case MFX_IMPL_HARDWARE:
			mfxRes = CreateConfig<mfxU32>(
				0, "mfxImplDescription.VendorImplID");
			break;
		case MFX_IMPL_HARDWARE2:
			mfxRes = CreateConfig<mfxU32>(
				1, "mfxImplDescription.VendorImplID");
			break;
		case MFX_IMPL_HARDWARE3:
			mfxRes = CreateConfig<mfxU32>(
				2, "mfxImplDescription.VendorImplID");
			break;
		case MFX_IMPL_HARDWARE4:
			mfxRes = CreateConfig<mfxU32>(
				3, "mfxImplDescription.VendorImplID");
			break;
		}
		...
		if (par.GPUCopy) {
			mfxRes =
				CreateConfig<mfxU16>(par.GPUCopy, "DeviceCopy");
			if (MFX_ERR_NONE != mfxRes)
				return MFX_ERR_UNSUPPORTED;
		}
		...
		if (par.NumExtParam) {
			for (mfxU32 idx = 0; idx < par.NumExtParam; idx++) {
				mfxRes = CreateConfig<mfxHDL>(par.ExtParam[idx],
							      "ExtBuffer");
				if (MFX_ERR_NONE != mfxRes)
					return MFX_ERR_UNSUPPORTED;
			}
...
public:
	explicit MFXVideoENCODE(mfxSession session) { m_session = session; }
	...
	{
		return MFXVideoENCODE_EncodeFrameAsync(m_session, ctrl, surface,
						       bs, syncp);
	}
...
public:
	explicit MFXVideoDECODE(mfxSession session) { m_session = session; }
	...
	virtual mfxStatus DecodeFrameAsync(mfxBitstream *bs,
					   ...
					   mfxSyncPoint *syncp) override
	{
		return MFXVideoDECODE_DecodeFrameAsync(
...
public:
	explicit MFXVideoVPP(mfxSession session) { m_session = session; }
	...
	virtual mfxStatus QueryIOSurf(mfxVideoParam *par,
				      mfxFrameAllocRequest request[2]) override
	{
	...
	virtual mfxStatus RunFrameVPPAsync(mfxFrameSurface1 *in,
					   ...
					   mfxSyncPoint *syncp) override
	{
		return MFXVideoVPP_RunFrameVPPAsync(m_session, in, out, aux,
						    syncp);
	}
...
public:
	...
	virtual ~MFXVideoDECODE_VPP(void) { Close(); }

	virtual mfxStatus Init(mfxVideoParam *decode_par,
			       mfxVideoChannelParam **vpp_par_array,
			       mfxU32 num_channel_par)
	{
		return MFXVideoDECODE_VPP_Init(m_session, decode_par,
					       vpp_par_array, num_channel_par);
	}
	virtual mfxStatus Reset(mfxVideoParam *decode_par,
				mfxVideoChannelParam **vpp_par_array,
				mfxU32 num_channel_par)
	{
		return MFXVideoDECODE_VPP_Reset(m_session, decode_par,
						vpp_par_array, num_channel_par);
	}
	virtual mfxStatus GetChannelParam(mfxVideoChannelParam *par,
					  mfxU32 channel_id)
	{
		return MFXVideoDECODE_VPP_GetChannelParam(m_session, par,
							  channel_id);
	}
	virtual mfxStatus DecodeFrameAsync(mfxBitstream *bs,
	...
	{
		return MFXVideoDECODE_VPP_DecodeFrameAsync(m_session, bs,
							   skip_channels,
							   num_skip_channels,
							   surf_array_out);
	}
