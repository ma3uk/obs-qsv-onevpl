...
void QSV_VPL_Encoder_Internal::LoadFrameData(mfxFrameSurface1*& surface,
	...
	if (surface->Info.FourCC == MFX_FOURCC_NV12) {
		...
		// load Y plane
		for (i = 0; i < h; i++) {
			avx2_memcpy(ptr + i * pitch, frame_data[0] + i * frame_linesize[0], line_size);
		}
		// load UV plane
		h /= 2;
		ptr = static_cast<mfxU8*>((surface_data->UV + surface_info->CropX +
			(surface_info->CropY / 2) * pitch));
		for (i = 0; i < h; i++) {
			avx2_memcpy(ptr + i * pitch, frame_data[1] + i * frame_linesize[1], line_size);
		}
	}
		...
		// load B plane
		for (i = 0; i < h; i++) {
			avx2_memcpy(surface_data->B + i * pitch, frame_data[0] + i * frame_linesize[0],
				line_size);
		}
	}
}
...
mfxStatus QSV_VPL_Encoder_Internal::Encode_tex(mfxU64 ts, uint32_t tex_handle,
	...
	*pBS = nullptr;
	...
	while (GetFreeTaskIndex(&nTaskIdx) == MFX_ERR_NOT_FOUND) {
		...
		avx2_memcpy(reinterpret_cast<uint8_t*>(&mfx_Bitstream), reinterpret_cast<uint8_t*>(&mfx_TaskPool[mfx_SyncTaskID].mfxBS),
			sizeof(mfxBitstream));
...
mfxStatus QSV_VPL_Encoder_Internal::Drain() {
	...
	for (int i = 0; i < mfx_TaskPoolSize; i++) {
		...
		if (sts >= MFX_ERR_NONE && mfx_TaskPool[i].syncp != nullptr) {
			sync_sts =
				MFXVideoCORE_SyncOperation(mfx_Session, mfx_TaskPool[i].syncp, INFINITE);
			if (sync_sts < MFX_ERR_NONE) {
