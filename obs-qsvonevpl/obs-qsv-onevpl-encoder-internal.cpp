...
QSV_VPL_Encoder_Internal::QSV_VPL_Encoder_Internal(mfxVersion &mfx_Version,
    ...
    : mfx_Impl(MFX_IMPL_HARDWARE_ANY), mfx_Platform({}), mfx_Version({}),
      mfx_Loader(0), mfx_LoaderConfig(), mfx_LoaderVariant(),
      ...
      mfx_TaskPool(), mfx_TaskPoolSize(0), mfx_SyncTaskID(),
      mfx_UseD3D11(isDGPU), b_isDGPU(false), mfx_EncParams({}),
  ...
  mfx_LoaderVariant[2].Data.U32 = MFX_ACCEL_MODE_VIA_D3D11;
  MFXSetConfigFilterProperty(
      mfx_LoaderConfig[2],
  ...
  mfx_LoaderConfig[2] = MFXCreateConfig(mfx_Loader);
  mfx_LoaderVariant[2].Type = MFX_VARIANT_TYPE_U32;
  mfx_LoaderVariant[2].Data.U32 = MFX_ACCEL_MODE_VIA_VAAPI;
  MFXSetConfigFilterProperty(
      mfx_LoaderConfig[2],
      reinterpret_cast<const mfxU8 *>("mfxImplDescription.AccelerationMode"),
      mfx_LoaderVariant[2]);
...
mfxStatus QSV_VPL_Encoder_Internal::Initialize(int deviceNum,
  ...
  // MFXSetConfigFilterProperty(
  //     mfx_LoaderConfig, reinterpret_cast<const mfxU8
  //     *>("mfxInitParam.GPUCopy"), mfx_LoaderVariant);
  ...
  MFXSetConfigFilterProperty(mfx_LoaderConfig[5],
                             reinterpret_cast<const mfxU8 *>(
  ...
  if (hw->device_handle == nullptr) {
    sts = hw->create_device(mfx_Session, deviceNum);
    if (sts < MFX_ERR_NONE) {
...
mfxStatus QSV_VPL_Encoder_Internal::Open(qsv_param_t *pParams,
  ...
  mfxStatus sts = Initialize(pParams->nDeviceNum, codec, nullptr);
  ...
  // mfx_Surface->FrameInterface->GetRefCounter(mfx_Surface,
  //                                            &mfx_SurfaceRefCounter);
...
mfxStatus QSV_VPL_Encoder_Internal::InitBitstream(enum qsv_codec codec) {
  ...
  mfxU32 mult = (codec == QSV_CODEC_AV1)    ? 340
                : (codec == QSV_CODEC_HEVC) ? 1000
                                            : 10;
  mfx_Bitstream.MaxLength =
      static_cast<mfxU32>(mfx_EncParams.mfx.BufferSizeInKB * 1000 * mult);
...
mfxStatus QSV_VPL_Encoder_Internal::Encode_tex(mfxU64 ts, uint32_t tex_handle,
  ...
  *pBS = nullptr;
  ...
  sts = hw->copy_tex(Texture, tex_handle, lock_key,
...
mfxStatus QSV_VPL_Encoder_Internal::Encode(mfxU64 ts, uint8_t **frame_data,
  ...
  *pBS = nullptr;
  ...
  mfx_EncSurface->Data.TimeStamp = ts;
  LoadFrameData(mfx_EncSurface, frame_data, frame_linesize);
...
mfxStatus QSV_VPL_Encoder_Internal::Drain() {
  ...
  for (int i = 0; i < mfx_TaskPoolSize; i++) {
    ...
    if (mfx_TaskPool[i].syncp != nullptr) {
      do {
        sync_sts =
            MFXVideoCORE_SyncOperation(mfx_Session, mfx_TaskPool[i].syncp, 100);
        if (sync_sts < MFX_ERR_NONE) {
...
mfxStatus QSV_VPL_Encoder_Internal::ClearData() {
  ...
  if (mfx_VideoEnc) {
    ...
    if (CurrentRefCount > mfx_EncSurfaceRefCount) {
      for (size_t i = 0; i < CurrentRefCount; i++) {
        if (mfx_EncSurface->Data.Locked == 0) {
          mfx_EncSurface->FrameInterface->Release(mfx_EncSurface);
        }
      }
    }
  ...
  if (mfx_UseTexAlloc == true && hw->encoder_counter <= 0) {
    hw->release_device();
    /*   Release();*/
  }
  return sts;
