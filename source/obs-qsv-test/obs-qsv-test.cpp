...
static bool has_encoder(mfxIMPL impl, mfxU32 codec_id)
{
	...
	if (sts == MFX_ERR_UNSUPPORTED) return false;
	...
	return sts != MFX_ERR_UNSUPPORTED;
}
...
static bool get_adapter_caps(IDXGIFactory *factory, uint32_t adapter_idx)
{
	...
	size_t video_memory = desc.DedicatedVideoMemory;
	mfxIMPL impl = impls[adapter_idx];

	caps.is_intel = true;
	caps.is_dgpu = dgpu;
	caps.supports_av1 = ((qsv_get_cpu_platform() > QSV_CPU_PLATFORM_RPL) || ((
		qsv_get_cpu_platform() == QSV_CPU_PLATFORM_UNKNOWN) && dgpu));
	caps.supports_hevc = ((qsv_get_cpu_platform() > QSV_CPU_PLATFORM_KBL) || ((
		qsv_get_cpu_platform() == QSV_CPU_PLATFORM_UNKNOWN) && dgpu));
